...
function downloadResume() {
  fetch("images/MirandaBradleyResume.pdf").then((response) => {
    response.blob().then((blob) => {
      let url = window.URL.createObjectURL(blob);
      let a = document.createElement("a");
      a.href = url;
      a.download = "MirandaBradleyResume.pdf";
      a.click();
    });
  });
}

function renderIcon(link, label, svgpath) {
  return (
    <Link href={link}>
